...
export const FrontHatchPanel = ({ createMaterial }: FrontHatchPanelProps) => (
  <group position={[0, 0.46, DOME_FRONT_Z]} rotation={[-0.28, 0, 0]}>
    <mesh castShadow receiveShadow material={createMaterial(HATCHERY_PALETTE.bronze, 'gold')}>
      <primitive attach="geometry" object={getBoxGeometry(0.62, 0.52, 0.075)} /></mesh>
    ...
    <mesh castShadow receiveShadow position={[0, 0, 0.072]} rotation={[Math.PI / 2, 0, 0]}>
      <primitive attach="geometry" object={getTorusGeometry(0.095, 0.022, 12, 28)} /></mesh>
    {PANEL_RIVET_OFFSETS.map((rivet, index) => (
      <mesh key={`hatch-rivet-${index}`} castShadow receiveShadow position={[rivet[0], rivet[1], 0.048]} material={createMaterial(HATCHERY_PALETTE.bronzeLight, 'gold')}>
        <primitive attach="geometry" object={getSphereGeometry(0.034, 14, 14)} /></mesh>
    ))}
  </group>
);
