...
export const BuildingVisual = ({
  ...
  const isGhost = materialMode === 'ghost-valid' || materialMode === 'ghost-invalid';
  ...
  if (family === 'town-hall') {
    return (
      <group>
        ...
        />
        <BeveledShell
          width={footprintX * 1.08}
          height={0.5}
          depth={footprintZ * 1.08}
          pivotY={2.05}
          color='#e6c37c'
          token='gold'
          cornerRadius={0.14}
        ...
        <mesh castShadow receiveShadow position={[0, 2.8, 0]}>
          <primitive
            attach='geometry'
            object={getConeGeometry(Math.max(0.85, Math.min(footprintX, footprintZ) * 0.34), 1.25, 4)}
          />
          {createMaterial('#7c4a2d', 'wood')}
        </mesh>
        <PerimeterRivetTrack
          width={footprintX}
          depth={footprintZ}
        ...
        />
        <MechanicalGauge
          position={[footprintX / 2 - 0.05, 0.95, footprintZ / 2 - 0.25]}
          rotationY={-Math.PI / 6}
          dialRadius={0.13}
          needleAngle={-Math.PI / 3.5}
          createMaterial={createMaterial}
        />
      </group>
    );
    ...
    return (
      <group>
        ...
        <mesh castShadow receiveShadow position={[0, 1, 0]}>
          <primitive
            attach='geometry'
            object={getConeGeometry(Math.max(0.22, Math.min(footprintX, footprintZ) * 0.45), 0.7, 6)}
          />
    ...
    return (
      <group>
        <mesh castShadow receiveShadow position={[0, 0.5, 0]}>
          <primitive
            attach='geometry'
            object={isMortar ? getCylinderGeometry(0.9, 1.05, 1, 12) : getCylinderGeometry(0.72, 0.84, 0.9, 10)}
          />
        ...
        <mesh castShadow receiveShadow position={[0, 1.15, 0]}>
          <primitive
            attach='geometry'
            object={isMortar ? getCylinderGeometry(0.62, 0.48, 0.75, 10) : getCylinderGeometry(0.42, 0.42, 0.72, 10)}
          />
          {createMaterial('#a2aab7', 'iron')}
        </mesh>
        <mesh castShadow receiveShadow position={[isMortar ? 0.62 : 0.56, 1.35, 0]}>
          <primitive
            attach='geometry'
            object={isMortar ? getCylinderGeometry(0.2, 0.2, 1.25, 10) : getCylinderGeometry(0.12, 0.12, 0.92, 10)}
          />
  ...
  if (family === 'decor') {
    return <MushroomTotemVisual level={level} footprintX={footprintX} footprintZ={footprintZ} createMaterial={createMaterial} />;
  }
  ...
  if (type === 'DEFENSE_LASER_TOWER') {
    return <LaserTowerVisual level={level} isActive={status === 'ACTIVE'} hp={hp} maxHp={maxHp} />;
  }
  ...
  return (
    <group>
      ...
      <mesh castShadow receiveShadow position={[0, 1.05, 0]}>
        <primitive attach='geometry' object={getCylinderGeometry(0.52, 0.52, 0.72, 12)} />
      ...
      {level >= 2 ? (
        <mesh castShadow receiveShadow position={[0, 1.75, 0]}>
          <primitive attach='geometry' object={getSphereGeometry(0.22 + Math.min(0.18, level * 0.03), 12, 12)} />
